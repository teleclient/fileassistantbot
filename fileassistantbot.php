    ...
    public function onUpdateNewMessage($update)
    {
        ...
        try {
            if (
                ...
                $sent_message = yield $this->messages->sendMessage([
                    'peer'            => $update,
                    'message'         => 'Generating download link… 0%',
                    'reply_to_msg_id' => $message_id
                ]);
                $time = time();
                $last_progress = 0;
                $queue_id = $this->randomString() . time();
                $output_file_name = yield $this->download_to_dir(
                    $update,
                                ...
                                try {
                                    yield $this->messages->editMessage(
                                        [
                                            'id'      => $sent_message['id'],
                                            'peer'    => $update,
                                            'message' => 'Generating download link… ' . $progress . '%'
                                        ],
                                        [
                                            'queue' => $queue_id
                                        ]
                ...
                yield $this->messages->editMessage(
                    [
                        'id'              => $sent_message['id'],
                        'peer'            => $update,
                        'message'         => 'Download link Generated in ' . (time() - $time) . " seconds!\n\n💾 " .
                            basename($output_file_name) . "\n\n📥 " . rtrim(WEBSERVER_BASE_URL, '/') . '/' .
                            str_replace(__DIR__ . '/', '', str_replace(' ', '%20', $output_file_name)) .
                ...
                if ($text == '/start') {
                    yield $this->messages->sendMessage([
                        'peer'            => $update,
                        'message'         => 'Hi! please send me any file url or file uploaded in Telegram and " . 
                            "I will upload to Telegram as file or generate download link of that file.',
                        'reply_to_msg_id' => $message_id
                    ]);
                    return;
                }
                if ($text == '/speedtest') {
                    if (
                        isset($this->latest_speedtest[$chat_id]) &&
                        time() - $this->latest_speedtest[$chat_id] < 3600
                    ) {
                        yield $this->messages->sendMessage([
                            'peer'            => $update,
                            'message'         => 'You can test speed once per hour.',
                            'reply_to_msg_id' => $message_id
                        ]);
                        return;
                    }
                    ...
                    $sent_message = yield $this->messages->sendMessage([
                        'peer'            => $update,
                        'message'         => 'Testing download and upload speed…',
                        'reply_to_msg_id' => $message_id
                    ]);
                    ...
                    if (preg_match_all('/(Down|Up)load:.*/', $output, $output)) {
                        ...
                        yield $this->messages->editMessage([
                            'id'      => $sent_message['id'],
                            'peer'    => $update,
                            'message' => 'Error while testing speed.'
                        ]);
                    }
                    return;
                }
                ...
                if ($url === false) {
                    yield $this->messages->sendMessage([
                        'peer'            => $update,
                        'message'         => 'URL format is incorrect. make sure your URL starts with either http:// or https://.',
                        'reply_to_msg_id' => $message_id
                    ]);
                ...
                if (empty($filename)) {
                    yield $this->messages->sendMessage([
                        'peer' => $update,
                        'message' => 'Can you check your URL? I\'m unable to detect filename from the URL.',
                        'reply_to_msg_id' => $message_id
                    ]);
                    return;
                }
                ...
                if ($filename_length > 60) {
                    yield $this->messages->sendMessage([
                        'peer' => $update,
                        'message' => 'Your filename contains ' . $filename_length . ' characters. Maximum limit allowed in Telegram is 60 characters. Please shorten your filename and try again.',
                        'reply_to_msg_id' => $message_id
                    ]);
                    return;
                }
                ...
                $promise = $client->request(
                    $url,
                    [
                        Amp\Artax\Client::OP_MAX_BODY_BYTES => (int) (1.5 * (1024 ** 3))
                    ]
                );
                ...
                if (empty($headers['content-length'][0])) {
                    yield $this->messages->sendMessage([
                        'peer'            => $update,
                        'message'         => 'Unable to obtain file size.',
                        'reply_to_msg_id' => $message_id
                    ]);
                    return;
                }
                ...
                if ($filesize > 1024 ** 3) {
                    yield $this->messages->sendMessage([
                        'peer'            => $update,
                        'message'         => 'Your file should be snakker than 1 GB.',
                        'reply_to_msg_id' => $message_id
                    ]);
                ...
                yield $this->messages->editMessage([
                    'id'      => $sent_message['id'],
                    'peer'    => $update,
                    'message' => "📤 Your request is in the queue. Do not send another request. Please be patient…\n🗂 File: " . $filename . "\n🔗 URL: " . $url . "\n💿 File Size: " . $this->formatBytes($filesize) . "\n\n⌛ Upload progress: 0%"
                ]);
                $time = time();
                $last_progress = 0;
                $queue_id = $this->randomString() . time();
                yield $this->messages->sendMedia([
                    ...
                    'media' => [
                        ...
                        'file' => new \danog\MadelineProto\FileCallback(
                            $filepath,
                            function ($progress) use ($update, $sent_message, &$last_progress, $filename, $filesize, $url, $queue_id) {
                                $progress = round($progress);
                                if ($progress > $last_progress + 4) {
                                    $last_progress = $progress;
                                    try {
